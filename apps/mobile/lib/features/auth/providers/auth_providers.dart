...
final planetProfileProvider = StreamProvider<PlanetModel?>((ref) async* {
  ...
  if (initialData != null) {
    ...
    // Returning users who genuinely don't have a profile yet (e.g. failed previous signup)
    // shouldn't be stuck in a long loop. We'll do one quick retry and then yield null.
    debugPrint('AUTH: Profile null for returning user. One quick retry...');
    ...
    try {
      ...
      if (initialData != null) {
        ...
        debugPrint('AUTH: Profile still null, assuming onboarding needed.');
        yield null;
      }
    } catch (_) {
      yield null;
    ...
    // New users yield null immediately to trigger onboarding
    yield null;
  }

  // 2. Yield updates from the realtime stream
  try {
    await for (final data in repo.watchProfile(user.id)) {
      final isLoginFlow = ref.read(isLoginFlowProvider);
      if (data == null && isLoginFlow) {
        // Only ignore null in stream if we are EXPLICITLY in a login flow
        // and expecting a profile to appear via sync.
        continue;
      }
...
final planetProfileByUidProvider = StreamProvider.family<PlanetModel?, String>((
  ...
  void updateCache(PlanetModel? model) {
    if (model != null) {
      Future.microtask(() {
        ...
        final existing = current[uid];

        // LOGIC: Only update if:
        // 1. We have no existing data OR
        // 2. The existing data is just a 'placeholder' (Explorer) and the new one is REAL data OR
        // 3. The new data is different and NOT a placeholder.
        bool isPlaceholder = model.xparqName == 'Explorer' && model.photoUrl.isEmpty;
        bool existingIsPlaceholder = existing != null &&
            existing.xparqName == 'Explorer' &&
            existing.photoUrl.isEmpty;

        if (existing == null || (existingIsPlaceholder && !isPlaceholder) || (!isPlaceholder && existing != model)) {
  ...
  try {
    ...
    if (model != null) {
      ...
      if (memoryCached != null &&
          !(memoryCached.xparqName == 'Explorer' &&
              memoryCached.photoUrl.isEmpty)) {
        yield memoryCached;
      } else {
        ...
        if (dbData != null) {
          ...
          // 3. Fallback to Memory Cache even if it's a placeholder, 
          // or create a new placeholder but DON'T overwrite if memoryCached exists
          yield memoryCached ?? PlanetModel.placeholder(uid);
        }
      }
    }
  } catch (e) {
  ...
  try {
    await for (final data in repo.watchProfile(uid)) {
      ...
      final cached = ref.read(profileCacheProvider)[uid];
      if (cached != null &&
          !(cached.xparqName == 'Explorer' && cached.photoUrl.isEmpty)) {
        yield cached;
      } else {
        // Only if absolutely no data exists, we allow the placeholder
  ...
  Future<void> signInWithGoogle() async {
    ...
    try {
      ...
      if (response.user != null) {
        ...
        // Check if user already has a planet profile (existing user vs new)
        final hasProfile = _ref.read(planetProfileProvider).valueOrNull != null;
        ...
        if (hasProfile) {
          ...
          // New user → go through onboarding (DOB → Planet)
          _ref.read(isLoginFlowProvider.notifier).state = false;
          state = state.copyWith(
            isLoading: false,
            step: AuthStep.initial, // Let router redirect to DOB
          );
        }
      } else {
        throw const AuthException('Invalid login credentials');
