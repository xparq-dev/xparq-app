...
class SupabaseAuthRepository {
  static const String _accountLifecycleFunction = 'account-lifecycle';
  ...
  Future<AuthResponse> signInWithGoogle() async {
    ...
    const iosClientId = AppConstants.googleIosClientId;
    ...
    if (response.user != null) {
      SignalSessionManager.instance.initialize().catchError((e) {
        debugPrint(
            '[AuthRepo] Post-Google login Signal initialization failed: $e');
      });
    }
  ...
  Future<void> deleteUserAccount() async {
    ...
    try {
      await _invokeAccountLifecycle(action: 'request_delete');
    } on FunctionException catch (error) {
      debugPrint(
        '[AuthRepo] account-lifecycle request_delete failed, using profile fallback: $error',
      );
      await _client.from('profiles').update({
  ...
  Future<void> restoreAccount(String uid) async {
    try {
      await _invokeAccountLifecycle(action: 'restore');
    } on FunctionException catch (error) {
      debugPrint(
        '[AuthRepo] account-lifecycle restore failed, using profile fallback: $error',
      );
      await _client.from('profiles').update({
  ...
  Future<void> permanentlyDeleteAccount(String uid) async {
    ...
    if (currentUid == null || currentUid != uid) {
      throw const AuthException(
        'Hard delete must be executed by the currently authenticated user.',
      );
    }

    await _invokeAccountLifecycle(action: 'purge_self');
    await _client.auth.signOut();
  }

  Future<Map<String, dynamic>> _invokeAccountLifecycle({
    required String action,
    Map<String, dynamic>? body,
    Map<String, String>? headers,
  }) async {
    final response = await _client.functions.invoke(
      _accountLifecycleFunction,
      body: {
        'action': action,
        ...?body,
      },
