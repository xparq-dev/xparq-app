-- Resolve GRST203 ambiguity by removing all overloaded versions of
-- public.send_chat_message and recreating one canonical function.
...
BEGIN
  ...
  ELSIF has_content_encrypted THEN
    content_column := 'content_encrypted';
  ELSE
    RAISE EXCEPTION
      'messages table must contain either content or content_encrypted column';
  END IF;
  ...
  IF has_message_type THEN
    insert_columns := insert_columns || ', message_type';
    insert_values :=
        insert_values || ', COALESCE(NULLIF($7, ''''), ''text'')';
  END IF;

  IF has_metadata THEN
    insert_columns := insert_columns || ', metadata';
...
GRANT EXECUTE ON FUNCTION public.send_chat_message(
  TEXT, UUID, TEXT, BOOLEAN, BOOLEAN, BOOLEAN, TEXT, TEXT, JSONB
