...
class _UserProfileScreenState extends ConsumerState<UserProfileScreen>
              ...
              builder: (context, opacity, child) {
                ...
                return SliverAppBar(
                  ...
                  expandedHeight: profile.isExpandedHeader
                      ...
                      : (isOwnProfile
                          ? 460
                          : 520), // Increased to ensure content clears both TabBar and Camera Notch
                  pinned: true,
                  stretch: true,
                  // Use solid background at 0.9 for rock-solid status bar icons
                  backgroundColor:
                      opacity >= 0.9 ? headerBaseColor : Colors.transparent,
                    ...
                    builder: (context, constraints) {
                      ...
                      return AnimatedOpacity(
                        ...
                        opacity: constraints.biggest.height <=
                                (topPadding + toolbarHeight + 48.0 + 10.0)
                            ? 1.0
                            : 0.0,
                        child: Text(
                          profile.xparqName,
                  ...
                  flexibleSpace: Stack(
                    ...
                    children: [
                      ...
                      // [FIX] Independent Solid Shield: Opaque layer above FlexibleSpaceBar content
                      // but behind Toolbar to prevent Bio text leakage during scroll.
                      Positioned.fill(
                        child: IgnorePointer(
                          child: Container(
                            // Solid background at 0.9 opacity for status bar safety
                            color: (opacity >= 0.9)
                                ? headerBaseColor
                                : Colors.transparent,
                          ),
                  ...
                  bottom: PreferredSize(
                    ...
                    child: Builder(
                      builder: (context) {
                        ...
                        return Container(
                          // Tab Bar must be fully opaque to prevent content bleed-through during scroll
                          color: opacity >= 0.9
                              ? headerBaseColor
                              : headerBaseColor.withValues(alpha: opacity),
                          child: tabBar,
                        );
                      },
                    ),
              ...
              child: FlexibleSpaceBar(
                background: Builder(
                  builder: (context) {
                    ...
                    final expandedHeight = isOwnProfile
                        ? 340.0
                        : 460.0; // Synchronized with monolith 340
                    final totalExpandedHeight = expandedHeight +
                        (profile.isExpandedHeader
                            ? (isOwnProfile ? 200.0 : 140.0)
                            : 0.0);
                    ...
                    final visibleT = ((top - (minPinnedHeight + 20.0)) /
                        ...
                        .clamp(0.0, 1.0);
                    final flexibleContentOpacity =
                        math.pow(visibleT, 1.8).toDouble();
                    ...
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                      if (mounted) {
                        ...
                        _headerOpacityNotifier.value =
                            (progress * 1.15).clamp(0.0, 1.0);
                      }
                    });
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  ...
                  style: TextStyle(
                    color: theme.colorScheme.onSurface,
                    fontSize: 20, // Standardized to match compact preference
    ...
    return ClipRRect(
      ...
      child: BackdropFilter(
        filter: ui.ImageFilter.blur(
          sigmaX: profile.isExpandedHeader ? 8 : 18,
          sigmaY: profile.isExpandedHeader ? 8 : 18,
        ),
        child: Container(
            ...
            color: profile.isExpandedHeader
                ...
                : (isDark
                    ? Colors.white.withValues(
                        alpha:
                            0.08) // Light-catching frost effect for dark mode
                    : Colors.white
                        .withValues(alpha: 0.6)), // Soft white for light mode
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 5), // Fixed spacing
              Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text(
  ...
  Widget _buildStats(BuildContext context, PlanetModel profile) {
    final sparkCount =
        ref.watch(userSparkCountProvider(profile.id)).valueOrNull ?? 0;
    final pulseCount =
        ref.watch(userPulseCountProvider(profile.id)).valueOrNull ?? 0;
    final orbitersCount = ref
            .watch(
                orbitCountProvider(OrbitListParams(profile.id, 'orbited_by')))
            .valueOrNull ??
        0;
    final orbitingCount = ref
            .watch(orbitCountProvider(OrbitListParams(profile.id, 'orbiting')))
            .valueOrNull ??
        0;
    final planetsCount = orbitersCount + orbitingCount;
      ...
      child: IntrinsicHeight(
        child: Row(
          ...
          children: [
            _StatColumn(
              label: 'pulses',
              value: _formatCount(pulseCount),
              icon: Icons.bubble_chart_outlined,
              customLabel: l10n.pulses,
            ),
            VerticalDivider(
              width: 12,
              thickness: 0.5,
            ...
            _StatColumn(
              label: 'spark',
              value: _formatCount(sparkCount),
              icon: Icons.auto_awesome,
              customLabel: l10n.spark,
            ),
            VerticalDivider(
              width: 12,
              thickness: 0.5,
            ...
            _StatColumn(
              label: 'planets',
              value: _formatCount(planetsCount),
              icon: Icons.public,
              customLabel: l10n.planets,
              onTap: () {
                context.push(
                  AppRoutes.orbitList,
                  extra: {
    ...
    return [
      ...
      Opacity(
        ...
        child: Align(
          ...
          child: SizedBox(
            width: double.infinity,
            height:
                210, // Reduced from 250 to make background area smaller (Reduced from 300 for half-overlap)
            child: Stack(
              children: [
                _buildCoverContent(
                  context,
      ...
      if (!_isRepositioningCover && !_isRepositioningAvatar)
        Positioned(
          ...
          bottom: 0,
          top:
              200, // Reduced from 240 to follow the smaller cover photo heighted at 250) by 30px for a solid card start
          child: Container(
            decoration: BoxDecoration(
      ...
      if (!_isRepositioningCover && !_isRepositioningAvatar)
        Positioned(
          bottom: isOwnProfile
              ? 95
              : 105, // Moved up even further for perfect balance
          left: 0,
          right: 0,
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              _buildAvatar(context, profile, isOwnProfile, isDark),
              const SizedBox(height: 8),
              Padding(
                ...
                Positioned(
                  ...
                  child: GestureDetector(
                    ...
                    onVerticalDragUpdate: (details) {
                      ...
                      } else if (profile.isExpandedHeader &&
                          !_isGalleryOpening) {
      ...
      Opacity(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const SizedBox(
                  height: 330), // Adjusted to prevent bottom overflow
              if (!(_isRepositioningCover || _isRepositioningAvatar)) ...[
                _buildIdentityInfo(
                  context,
                  profile,
      ...
      child: Stack(
        ...
        children: [
          ...
          if (isOwnProfile && !_isRepositioningCover && !_isRepositioningAvatar)
            Align(
              alignment: Alignment
                  .center, // Vertically centered in the cover image area
              child: Container(
                padding: const EdgeInsets.symmetric(
                  horizontal: 16,
                  vertical: 8,
        ...
        children: [
          ...
          _buildSection(
            ...
            child: Row(
              ...
              children: [
                ...
                const SizedBox(width: 20), // Wider spacing for balance
                _PhotoAlbumFolder(
              ...
              children: [
                ...
                if (profile.contactPhone != null &&
                  ...
                  _buildInfoRow(
                    ...
                    trailing: (profile.contactEmail == null ||
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(
                              height: 2), // Minimal gap in Expanded mode.
                          Padding(
          ...
          if (profile.links.isNotEmpty)
            _buildSection(
              ...
              child: Column(
                children:
                    profile.links.map((link) => _LinkTile(url: link)).toList(),
              ),
            ),
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              ...
              fontWeight: FontWeight.bold,
              letterSpacing:
                  0.0, // Removed 1.2 spacing to prevent Thai character disconnection
            ),
          ),
          const SizedBox(height: 12),
          child,
...
class _ProfileStatWithAvatars extends ConsumerWidget {
  final String label;
  final String uid;
  final String collection;
  final String customLabel;

  const _ProfileStatWithAvatars({
    required this.label,
    required this.uid,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Fetch actual users in this list to get real photos
      ...
      child: Row(
        ...
        children: [
          ...
          membersAsync.when(
            ...
            loading: () => const _AvatarStackPlaceholder(count: 2),
            error: (_, __) => const _AvatarStackPlaceholder(count: 0),
          ),
        ],
      ),
    );
...
class _AvatarStackReal extends StatelessWidget {
  final List<String> photos;
  final int count;
  const _AvatarStackReal({required this.photos, required this.count});

  @override
  Widget build(BuildContext context) {
    if (count == 0) return const SizedBox.shrink();

    // Determine how many to show (max 3 or actual count)
    final displayCount = math.min(count, 3);

    return SizedBox(
      width: 14.0 + (displayCount - 1) * 12.0,
      height: 24,
      child: Stack(
...
class _AvatarStackPlaceholder extends StatelessWidget {
  final int count;
  const _AvatarStackPlaceholder({required this.count});

  @override
  Widget build(BuildContext context) {
    if (count <= 0) return const SizedBox.shrink();
    return SizedBox(
      width: 14.0 + (count - 1) * 12.0,
      height: 24,
      child: Stack(
        children: List.generate(math.min(count, 3), (index) {
          return Positioned(
            left: index * 12.0,
            child: Container(
              width: 24,
              ...
              child: const Icon(Icons.person, size: 12, color: Colors.white24),
            ),
          );
        }),
      ),
