          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildAuthOption(
                  ...
                  onTap: () async {
                    ...
                    try {
                      ...
                      // After successful Google sign-in, check profile
                      final authState = ref.read(authNotifierProvider);
                      if (authState.step == AuthStep.complete) {
                        // Existing user with profile → router handles it
                      } else {
                        // New user → go to DOB onboarding
                        if (context.mounted) {
                          context.push(AppRoutes.dobInput);
                        }
                      }
                    } catch (e) {
                      if (context.mounted) {
                ...
                // Phone Option (Coming Soon / Disabled)
                _buildAuthOption(
                  context: context,
                  ...
                  onTap: () {
                    HapticFeedback.lightImpact();
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Phone authentication is coming soon!')),
                    );
                  },
                ),
                const SizedBox(height: 16),
