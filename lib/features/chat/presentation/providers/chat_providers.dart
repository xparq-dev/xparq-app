...
final chatProfileProvider = StreamProvider.autoDispose
  ...
  final ciphertextCacheById = <String, String>{};
      ...
      return repo
          ...
          .asyncMap((messages) async {
            ...
            if (validMessages.isEmpty && messages.isNotEmpty) {
              return <MessageModel>[];
            }


            // 1) Fetch all cached plaintexts pre-emptively
            final messageIds = validMessages.map((m) => m.messageId).toList();
            final persistentCache = await OfflineChatDatabase.instance
                .getSignalMessageCache(messageIds);

            // 2) SYNC-FIRST RESOLUTION: Resolve everything we already know (O(N) sync)
            // This is key for 0ms confirming of rapid bursts.
            final syncProcessed = validMessages.map((message) {
              final messageId = message.messageId;
              final rawCiphertext = message.content;

              // A. Memory Cache
              if (ciphertextCacheById[messageId] == rawCiphertext && decryptedCacheById[messageId] != null) {
                return message.copyWith(decryptedContent: decryptedCacheById[messageId]);
              }
              ...
              if (myUid != null && message.senderUid == myUid) {
                ...
                final localPlaintext = MessageEncryptionService.resolveOutgoingPlaintext(
              ...
              if (SignalSessionManager.isBroken) {
                if (myUid != null && message.senderUid == myUid) {
                  ...
                  const placeholder = '🔒 Encrypted message (Signal service unavailable)';
                  ciphertextCacheById[messageId] = rawCiphertext;
            ...
            final processed = await Future.wait(syncProcessed.map((message) async {
              ...
              String plaintext;
              try {
                plaintext = await MessageEncryptionService.decrypt(rawCiphertext, chatId);
              } catch (e) {
              ...
              if (plaintext != rawCiphertext && plaintext.isNotEmpty) {
                await OfflineChatDatabase.instance.cacheSignalMessage(messageId, plaintext, rawCiphertext);
              }
              return message.copyWith(decryptedContent: plaintext);
            }));
        ...
        data: (messages) {
          ...
          for (final m in mappedMessages) {
            ...
            if (text.isNotEmpty) {
               ...
               final fuzzyKey = '${m.senderUid}_${text}_${m.timestamp.millisecondsSinceEpoch ~/ 15000}';
               dbByFuzzyKey[fuzzyKey] = m;
            }
          }

          final pendingSorted = List<MessageModel>.from(pendingForChat)..sort(
            (a, b) => a.timestamp.compareTo(b.timestamp),
          );

          for (final pm in pendingSorted) {
            final text = pm.decryptedContent ?? pm.content;
            final fuzzyKey = '${pm.senderUid}_${text}_${pm.timestamp.millisecondsSinceEpoch ~/ 15000}';
            ...
            if (match != null) {
              ...
              final idx = merged.indexWhere(
                (m) => m.messageId == match.messageId || m.messageId == pm.messageId,
              );
